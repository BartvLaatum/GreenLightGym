...
if __name__ == "__main__":
    ...
    names = ["vital-yogurt-9","robust-lake-21", "fancy-waterfall-16"]
    labels = ["No constraints", "Scaled reward", r"Tuned $c_i$"]
    markers = ["o", "^", "v"]
    models = [PPO.load(f"trainData/{args.project}/models/{runname}/best_model.zip", env=env) for runname in names]
    cmap = 'coolwarm'
    ...
    for i, model in enumerate(models[:-1]):
        ...
        axs[0].scatter(range(0, len(model_actions)), model_actions, c=model_actions, cmap = cmap, norm=norm, label=labels[i], marker=markers[i], alpha=0.8)
        axs[0].set_ylabel('Action')

        axs[1].scatter(range(0,len(shap_values[:,0])), meanObs[:,0], c=shap_values[:,0], cmap=cmap, norm=norm, label=labels[i], marker=markers[i], alpha=0.8)
        axs[1].set_ylabel("Air temp")

        axs[2].scatter(range(0,len(shap_values[:,1])), meanObs[:,1], c=shap_values[:,1], cmap=cmap, norm=norm, label=labels[i], marker=markers[i], alpha=0.8)
        axs[2].set_ylabel("CO2 conc")

        axs[3].scatter(range(0,len(shap_values[:,2])), meanObs[:,2], c=shap_values[:,2], cmap=cmap, norm=norm, label=labels[i], marker=markers[i], alpha=0.8)
        axs[3].set_ylabel("Humidity")

        axs[4].scatter(range(0,len(shap_values[:,3])), meanObs[:,3], c=shap_values[:,4], cmap=cmap, norm=norm, label=labels[i], marker=markers[i], alpha=0.8)
        axs[4].set_ylabel("Weight")
        
        axs[5].scatter(range(0,len(shap_values[:,4])), meanObs[:,4], c=shap_values[:,4], cmap=cmap, norm=norm, label=labels[i], marker=markers[i], alpha=0.8)
        axs[5].set_ylabel("Harvest")

        axs[6].scatter(range(0,len(shap_values[:,5])), meanObs[:,5], c=shap_values[:,5], cmap=cmap, norm=norm, label=labels[i], marker=markers[i], alpha=0.8)
