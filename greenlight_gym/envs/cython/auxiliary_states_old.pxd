...
# from libc.math cimport cos, M_PI, exp, sqrt, fabs, fmax, fmin, log, M_E, pi, floor
...
cdef packed struct AuxiliaryStates:
    ...
    double timeOfDay    # hours since midnight time of day [h]
    double dayOfYear           # days since start of the year [d]
    double rhIn         # relative humidity inside [%]   
    double mcFruitHarSum  # Harvested fruit                       [mg m^-2 timestep^-1]
...
cdef inline void update(AuxiliaryStates* a, Parameters* p, double* u, double* x, double* d):
    ...
    Args:
        ...
        0: shScr         closure of shading screen (0 is open 1 is closed)
        0: shScrPer      closure of semi-permanent shading screen (0 is open 1 is closed)
        0: side          side vent aperature (0 is closed 1 is open)
    ...
    # Equation 18 [1]
    a.capCov = cos(radToDegrees(p.psi)) * (0 * p.hShScrPer * p.rhoShScrPer * p.cPShScrPer + p.hRf * p.rhoRf * p.cPRf)
    ...
    a.cD = p.cDgh * (1 - p.etaShScrCd*0)

    # Discharge coefficient [-]
    # Equation 74 [-]
    # addAux(gl, 'cW', p.cWgh*(1-p.etaShScrCw*u.shScr))
    a.cW = p.cWgh * (1 - p.etaShScrCw*0)
    ...
    a.hCovInCovE = sensible(\
        1/(p.hRf/p.lambdaRf + 0*p.hShScrPer/p.lambdaShScrPer),\
        x[5], x[6])
